...
class Vpc {
	...
	private function define_admin_hooks() {
		...
		$config=new VPC_Config(FALSE);
                ...
                $this->loader->add_action( 'save_post_product', $config, 'save_config');
...
//                $this->loader->add_action( 'woocommerce_product_write_panels', $config, 'get_product_tab_data');
	...
	private function define_public_hooks() {
		...
		$this->loader->add_action( 'wp_enqueue_scripts', $plugin_public, 'enqueue_scripts' );
                ...
                $this->loader->add_filter("woocommerce_cart_item_name", $plugin_public, "get_vpc_data", 10, 3);
                $this->loader->add_action( 'woocommerce_before_calculate_totals', $plugin_public, 'get_cart_item_price', 10 );
                $this->loader->add_action( 'woocommerce_before_cart_item_quantity_zero', $plugin_public, 'remove_config_from_cart');
                $this->loader->add_action( 'woocommerce_remove_cart_item', $plugin_public, 'remove_config_from_cart');
                $this->loader->add_action( 'woocommerce_add_order_item_meta', $plugin_public, 'save_customized_item_meta',10,3);
                
                //User my account page
                $this->loader->add_filter( 'woocommerce_order_item_quantity_html', $plugin_public, 'get_user_account_products_meta',11,2);
